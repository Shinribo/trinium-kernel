...
impl<T> RwSpinlock<T> {
    ...
    pub fn is_locked_read(&self) -> bool {
        ...
        self.ticket_queue.load(Ordering::Acquire) != self.ticket_enter.load(Ordering::Acquire) && self.ticket_queue.load(Ordering::Acquire) != self.ticket_exit.load(Ordering::Acquire)
